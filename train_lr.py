"""
Train Logistic Regression model for MAGPIE-Lite.
"""
import os
import sys
import yaml
...
def train_logistic_regression() -> None:
    ...
    model_path = os.path.join(config['output']['models_dir'], f'lr_model_{timestamp}.pkl')
    model.save(model_path)
    logger.info(f"Model saved to {model_path}")
    
    # Save feature importances
    importances = model.get_feature_importances()
    importances_path = os.path.join(config['output']['models_dir'], 'lr_feature_importances.csv')
